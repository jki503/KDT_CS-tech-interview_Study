...
- [불연속 메모리 할당](#불연속-메모리-할당)
- [페이징](#페이징)
  - [논리 주소와 페이지 매핑 테이블](#논리-주소와-페이지-매핑-테이블)
  - [page & offset](#page--offset)
  - [페이징 장단점](#페이징-장단점)
- [세그멘테이션](#세그멘테이션)
  - [segment & offset](#segment--offset)
  - [세그멘테이션 장단점](#세그멘테이션-장단점)

</br>

## 불연속 메모리 할당

> 프로그램의 일부가 서로 다른 주소 공간에 할당 될 수 있는 기법이다.  
> 연속 메모리 할당에서 발생할 수 있는 단편화 문제를 해결하기 위해 제시된 기법으로  
...
> 프로세스를 일정한 크기의 페이지로 분할하여 메모리에 적재하는 방식이다.  
> `페이지` : 고정 사이즈의 가상 메모리 내의 프로세스 조각이다.  
> `프레임` : 페이지 크기와 같은 Main memory의 메모리 조각이다.
